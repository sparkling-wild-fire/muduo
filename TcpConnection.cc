...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0)  // buff可读（发送缓冲区）为0才能发送？ =》 因为上一次的还没发送出去
    {
        ...
        if (nwrote >= 0)       // 发送成功了
        {
            ...
            {
                // 既然在这里数据全部发送完成，就不用再给channel设置epollout事件了   =》 handlewrite?
                loop_->queueInLoop(
...
void TcpConnection::shutdownInLoop()
{
    ...
    {
        socket_->shutdownWrite(); // 关闭写端     =》 关闭  回调  所有的方法都注释掉了
    }
}
...
void TcpConnection::handleWrite()
{
    if (channel_->isWriting())
    {
        ...
        if (n > 0)
        {
            ...
            {
                channel_->disableWriting();    // 不可写？ 把buff数据写完了，=》为啥就不可写了
                if (writeCompleteCallback_)    // 如果有对应回调
                {
...
void TcpConnection::handleClose()
{
    ...
    closeCallback_(connPtr); // 关闭连接的回调  执行的是TcpServer::removeConnection回调方法  =》 这两个回调啥区别
}
